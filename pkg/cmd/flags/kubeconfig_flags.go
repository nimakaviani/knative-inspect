...
func (f *KubeconfigFlags) Set(cmd *cobra.Command) {
	...
	cmd.PersistentFlags().Var(f.Path, "kubeconfig", "Path to the kubeconfig file ($KUBECONFIG)")

	f.Context = NewKubeconfigContextFlag()
	cmd.PersistentFlags().Var(f.Context, "kubeconfig-context", "Kubeconfig context override ($KUBECONFIG_CONTEXT)")
}
...
func (s *KubeconfigContextFlag) Resolve() error {
	...
	s.value = os.Getenv("KUBECONFIG_CONTEXT")
